...
6. **NDVI satelital**: correr `supabase/sql/safia_ndvi.sql` y `supabase/sql/safia_planes.sql` en el SQL Editor; crear una cuenta gratis en https://dataspace.copernicus.eu → Dashboard → User settings → OAuth clients → Create; cargar `CDSE_CLIENT_ID` y `CDSE_CLIENT_SECRET` en Edge Functions → Secrets del proyecto nuevo.
