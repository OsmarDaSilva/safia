...
(function () {
  ...
  function html(caso, cont) {
    ...
    h += '<h3 style="font-size:14px;margin:14px 0 6px;">Qué hay que hacer (tildá lo que ya se hizo)</h3><div class="tablewrap"><div class="tablescroll"><table class="tbl"><thead><tr><th></th><th>Ítem</th><th>Objetivo</th><th>Qué hacer</th><th class="r">US$/ha</th><th>Hecho el</th></tr></thead><tbody>' +
      items.map(function (i) { return '<tr' + (i.hecho ? ' style="background:#E7F6EA;"' : '') + '><td><input type="checkbox" class="segChk" data-k="' + esc(i.k) + '"' + (i.hecho ? ' checked' : '') + '></td><td><b>' + esc(i.nombre) + '</b><div class="sub">' + esc(i.tipo || '') + '</div></td><td style="font-size:12px;">' + esc(i.objetivo) + '</td><td style="font-size:12px;">' + esc(i.accion) + '</td><td class="r">' + fmt(i.costo) + '</td><td>' + (i.fechaHecho ? fmtF(i.fechaHecho) : '<span class="muted">—</span>') + '</td></tr>'; }).join('') + '</tbody></table></div></div>';
    ...
    indicadorAgua(caso, u).then(function (ag) {
      ...
      ind.innerHTML = '<div class="note ' + (kG === 'rojo' || kG === 'ambar' ? 'warn' : 'info') + '" style="border-left-color:' + COLOR[kG] + ';">' + semaforo(kG, 'Estado general') + ' · ' + esc(txtG) + '</div>' +
        '<div class="tablewrap"><div class="tablescroll"><table class="tbl"><tbody>' + filas.map(function (f) { return '<tr><td style="width:190px;">' + semaforo(f[1].k, f[0]) + '</td><td style="font-size:12.5px;">' + (f[1].texto || '') + '</td></tr>'; }).join('') + '</tbody></table></div></div>';
    });
  }
