...
(function () {
  ...
  function plan(caso, meta, pr, casos, analisisProfundos, opciones) {
    ...
    if (opciones.calcareoTnHa > 0) {
      ...
      item({ k: 'encalado', opcional: !cal.necesita, tipo: 'suelo', nombre: 'Encalado', hoy: (v != null ? 'V% ' + fmt(v, 1) : '') + (ph != null ? ' · pH ' + fmt(ph, 1) : '') + (num(s.phSmp) != null ? ' · SMP ' + fmt(num(s.phSmp), 1) : ''), objetivo: 'pH 6,0 (V% ' + vObj + ')' + (bm && bm.suelo.satBases ? ' · los que rinden ≥ meta: ' + fmt(bm.suelo.satBases, 0) : ''),
    ...
    [['boro', 'boro', 'Boro', 'boroUSDkg', 'al suelo (bórax o ulexita, dura 4–5 años) o foliar en floración; no exceder', [0.03, 0.08]], ['zinc', 'zinc_suelo', 'Zinc (suelo)', 'znSueloUSDkg', 'al suelo (sulfato de zinc, dura 4–5 años) o en semilla + foliar', [0.02, 0.06]], ['cobre', 'cobre', 'Cobre', 'cuUSDkg', 'al suelo (sulfato de cobre) o foliar', [0.01, 0.04]], ['manganeso', 'manganeso', 'Manganeso', 'cuUSDkg', 'foliar en V4–R1 o al suelo', [0.01, 0.03]]].forEach(function (d) {
      ...
      var yaAplic = caso.manejo && caso.manejo.npk ? ((d[0] === 'boro' ? caso.manejo.npk.b : (d[0] === 'zinc' ? caso.manejo.npk.zn : 0)) || 0) : 0;   // ya aplicado al suelo en esta campaña (ulexita, bórax, sulfato de zinc)
      if (yaAplic >= im.dosis) return;
      item({ k: d[1], opcional: im.clase === 'medio', tipo: 'suelo', nombre: d[2], hoy: fmt(val, 2) + ' mg/dm³ (' + im.clase + ')' + (yaAplic ? ' · ya aplicaste ' + fmt(yaAplic, 1) + ' kg/ha esta campaña' : ''), objetivo: '≥ ' + fmt(im.limites[1], 2) + ' mg/dm³ (Embrapa 2013)', accion: im.dosis + ' kg/ha ' + d[4] + (im.clase === 'medio' ? ' (dosis de "medio": para alto rinde)' : ''), inversion: im.dosis * (pr[d[3]] || 6) + 6, vidaUtil: 4, aporteMin: im.clase === 'bajo' ? d[5][1] / 2 : d[5][0], aporteMax: im.clase === 'bajo' ? d[5][1] : d[5][0] * 2, fuente: '[16] Embrapa 2013 / Fundação MS Tabelas 21–22' });
    });
    ...
    if (rot.cargada && !rot.conCobertura) item({ k: 'cobertura', opcional: true, tipo: 'manejo', nombre: 'Cobertura de invierno', hoy: 'sin cobertura', objetivo: (bm && bm.cobertura != null ? fmt(bm.cobertura * 100, 0) + ' % de los que rinden ≥ meta usan cobertura' : 'brachiaria, avena o mix'), accion: 'Sembrar cobertura después de la cosecha (brachiaria ruziziensis, avena, mix)', recurrente: pr.coberturaUSDha, alcance: 'lote', aporteMin: 0.05, aporteMax: 0.15, fuente: '[10][11] Embrapa +15 %, Fundação MS +17 %' });
    ...
    if (nFung != null && nFung < fungObj) item({ k: 'fungicidas', opcional: true, tipo: 'manejo', nombre: 'Fungicidas', hoy: nFung + ' aplicación(es)', objetivo: fungObj + '+ (' + (cu === 'soja' ? 'roya y mancha' : 'manchas foliares') + ')', accion: 'Sumar ' + (fungObj - nFung) + ' aplicación(es) preventiva(s) en R1–R5', costo: (fungObj - nFung) * pr.fungicidaUSDapl, aporteMin: 0.05, aporteMax: 0.15, fuente: 'Embrapa Soja: la roya sin control pierde hasta 90 %; 2–3 aplicaciones preventivas' });
    /* 8. Agua */
    ...
    if (!opciones.preparacionCompleta && !opciones.construirPK) {
      var fijos = items.filter(function (i) { return !i.opcional; }), opc = items.filter(function (i) { return i.opcional; });
      var cubierto = fijos.reduce(function (a, i) { return a + (i.condicional ? 0 : (i.aporteMin || 0)); }, 0) * actual;
      var costoAnual = function (i) { return (i.inversion ? i.inversion / Math.max(1, i.vidaUtil || 1) : 0) + (i.recurrente || 0) + (i.costo || 0); };
      opc.sort(function (a, b) { return ((b.aporteMin || 0) * actual) / (costoAnual(b) + 1) - ((a.aporteMin || 0) * actual) / (costoAnual(a) + 1); });
      var elegidos = [];
      opc.forEach(function (i) { if (cubierto < gapKg * 1.3) { elegidos.push(i); cubierto += (i.aporteMin || 0) * actual; } else omitidos.push(i); });
      items = items.filter(function (i) { return !i.opcional || elegidos.indexOf(i) >= 0; });
    }
