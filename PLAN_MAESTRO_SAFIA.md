...
**Usuarios con nombre y rol (23-sep-2026)** — Login: Supabase Auth con correo y contraseña (login.html); los usuarios los crea Irrigar en Authentication → Users. Nueva tabla `safia_usuarios` (supabase/sql/safia_usuarios.sql, PENDIENTE que Osmar la corra): id = auth.uid(), email, nombre, rol (admin / cliente / operador), cliente_id; se llena sola al crear un usuario (trigger) y para los existentes; `safia_es_admin()` security definer para las políticas. safia-sync.js publica `localStorage.safia_usuario` + evento `safia:usuario` y expone `SafiaSync.usuario()/esAdmin()`; sin tabla, el nombre sale del correo. El Dashboard saluda por nombre. Hoy TODOS los usuarios autenticados ven todo (RLS "authenticated" en todas las tablas): los roles todavía no filtran; ese es el próximo paso (cliente ve solo sus campos, operador solo carga eventos), que requiere antes el sync por registro.
