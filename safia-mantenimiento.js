...
(function () {
  ...
  /* ---------- contadores de horas ----------
     FieldNET muestra por separado el tiempo del pivote, de la bomba y del cañón final. Una lectura sin "contador" es del pivot. */
  var CONTADORES = { pivote: 'Pivot', bomba: 'Bomba', canon: 'Cañón final' };
  function contadorDe(ev) { return ev && CONTADORES[ev.contador] ? ev.contador : 'pivote'; }
  function contadorTarea(t) { var c = String(t && t.componente || '').toLowerCase(); return /bomba|motor/.test(c) ? 'bomba' : /ca[ñn][óo]n/.test(c) ? 'canon' : 'pivote'; }
  ...
  function estado(eq, eventos) {
    ...
    evs.filter(function (e) { return e.tipo === 'horimetro' && contadorDe(e) === 'pivote' && num(e.cantidad) != null; }).forEach(function (e) { if (num(e.cantidad) >= horas) { horas = num(e.cantidad); fuenteHoras = 'horímetro'; fechaHoras = String(e.fecha || '').slice(0, 10); } });
    // bomba y cañón final tienen su propio contador (FieldNET los separa); si no hay lectura, se usan las horas del pivot
    var contadores = { pivote: { horas: horas, fecha: fechaHoras, propio: true } };
    ['bomba', 'canon'].forEach(function (c) {
      var mx = null, fx = null;
      evs.filter(function (e) { return e.tipo === 'horimetro' && contadorDe(e) === c && num(e.cantidad) != null; }).forEach(function (e) { if (mx == null || num(e.cantidad) >= mx) { mx = num(e.cantidad); fx = String(e.fecha || '').slice(0, 10); } });
      contadores[c] = mx == null ? { horas: horas, fecha: fechaHoras, propio: false } : { horas: mx, fecha: fx, propio: true };
    });
    var plan = (eq && eq.planMantenimiento) || [];
    if (!plan.length) return { sinPlan: true, horas: horas, fuenteHoras: fuenteHoras, fechaHoras: fechaHoras, contadores: contadores, tareas: [], vencidas: [], proximas: [] };
    ...
    var tareas = plan.map(function (t) {
      ...
      var cont = contadorTarea(t), horasT = contadores[cont].propio ? contadores[cont].horas : horas, contLect = contadores[cont].propio ? cont : 'pivote';
      var intervalo = num(cada.horas) || 0, ultHoras = num(t.ultimasHorasHechas) || 0, ultFecha = t.ultimaFechaHecha || null, ultPor = null;
      if (!ultHoras && !ultFecha && !(t.historial && t.historial.length) && t.creado) { var primera = evs.filter(function (e) { return e.tipo === 'horimetro' && contadorDe(e) === contLect && num(e.cantidad) != null && String(e.fecha || '').slice(0, 10) >= String(t.creado).slice(0, 10); }).sort(function (a, b) { return String(a.fecha).localeCompare(String(b.fecha)); })[0]; if (primera) ultHoras = num(primera.cantidad) - (primera.desde && String(primera.desde).slice(0, 10) >= String(t.creado).slice(0, 10) ? (num(primera.horasPeriodo) || 0) : 0); }
      evs.filter(function (e) { return e.tipo === 'mantenimiento' && (String(e.tareaId) === String(t.id) || (!e.tareaId && e.tarea === t.tarea)); }).forEach(function (e) {
        var h = num(e.horasContador != null ? e.horasContador : e.horasEquipo); if (h == null) h = 0;
      ...
      if (intervalo > 0) { var ph = (horasT - ultHoras) / intervalo * 100; pcts.push(ph); faltaTxt = intervalo - (horasT - ultHoras) >= 0 ? 'faltan ' + fmt(intervalo - (horasT - ultHoras), 0) + ' h' : 'pasada por ' + fmt((horasT - ultHoras) - intervalo, 0) + ' h'; }
      var sinVueltas = false;
      if (num(cada.vueltas)) { if (horasPorVuelta) { var hv = cada.vueltas * horasPorVuelta; pcts.push((horasT - ultHoras) / hv * 100); } else sinVueltas = true; }
      ...
      return { id: t.id, k: t.k || null, tarea: t.tarea, componente: t.componente || '', intervalo: intervalo, cada: cada, cadaTxt: textoCada(cada), detalle: t.detalle || '', fuente: t.fuente || '', ultHoras: ultHoras, ultFecha: ultFecha, ultPor: ultPor, pct: pct == null ? null : Math.round(pct), estado: est, faltaTxt: faltaTxt, sinVueltas: sinVueltas, faltan: intervalo > 0 ? Math.round(intervalo - (horasT - ultHoras)) : null, contador: cont, horasContador: horasT };
    });
    return { sinPlan: false, horas: horas, fuenteHoras: fuenteHoras, fechaHoras: fechaHoras, contadores: contadores, tareas: tareas, completar: tareas.filter(function (t) { return t.estado === 'completar'; }), vencidas: tareas.filter(function (t) { return t.estado === 'vencido'; }), proximas: tareas.filter(function (t) { return t.estado === 'proximo'; }) };
  }
  ...
  function registrarHoras(eqId, horas, fecha, contador) {
    horas = num(horas); if (horas == null || horas < 0) return { error: 'Poné las horas que marca el horímetro.' };
    var ev = { id: Date.now() + Math.floor(Math.random() * 1000), equipoId: isNaN(parseInt(eqId)) ? eqId : parseInt(eqId), tipo: 'horimetro', fecha: fecha || hoy(), cantidad: horas, unidad: 'h', cargadoPor: quien(), fechaCreacion: new Date().toISOString() };
    if (contador && contador !== 'pivote' && CONTADORES[contador]) ev.contador = contador;
    return { ok: guardarEvento(ev) };
  }
  function registrarHecha(eqId, tareaId, horasEquipo, fecha, obs) {
    ...
    var s = estado(eq), et = s.tareas.find(function (x) { return String(x.id) === String(t.id); }), h = num(horasEquipo); if (h == null) h = s.horas;
    var hc = et && et.contador !== 'pivote' ? et.horasContador : null;
    return { ok: guardarEvento({ id: Date.now(), equipoId: eq.id, tipo: 'mantenimiento', fecha: fecha || hoy(), tareaId: t.id, tarea: t.tarea, componente: t.componente || '', horasEquipo: h, horasContador: hc, contador: et ? et.contador : 'pivote', observaciones: obs || '', cargadoPor: quien(), fechaCreacion: new Date().toISOString() }) };
  }
  ...
  function html(eq, opciones) {
    ...
    var cs = s.contadores || {}, btnH = '<button type="button" class="mt-btn" onclick="SafiaMant._horas(\'' + esc(String(eq.id)) + '\',\'' + id + '\')">';
    if (eq.tipo === 'pivote') {
      h += '<div class="mt-horas" style="gap:14px;">' + ['pivote', 'bomba', 'canon'].map(function (c) { var x = cs[c] || {}; return '<span>' + CONTADORES[c] + ': <b>' + (x.propio && (c === 'pivote' ? s.fuenteHoras : true) ? fmt(x.horas, 0) + ' h' : '—') + '</b>' + (x.propio && x.fecha ? ' <span style="color:#8C9196;font-size:11px;">' + fmtF(x.fecha) + '</span>' : '') + '</span>'; }).join('') + '</div>' +
        '<div class="mt-horas"><input type="number" step="1" min="0" placeholder="pivot h" id="' + id + 'Horas" title="Tiempo de ejecución del pivote"><input type="number" step="1" min="0" placeholder="bomba h" id="' + id + 'HorasBomba" title="Tiempo de ejecución de la bomba"><input type="number" step="1" min="0" placeholder="cañón h" id="' + id + 'HorasCanon" title="Tiempo del cañón final">' + btnH + 'Cargar lecturas</button></div>' +
        '<div class="mt-vacio" style="margin:-4px 0 8px;">Están en FieldNET → Historial → Totales de tiempo de ejecución. Las horas de la bomba y del cañón cuentan para sus propias tareas.</div>';
    } else h += '<div class="mt-horas"><span>Horímetro: <b>' + fmt(s.horas, 0) + ' h</b></span>' + (s.fechaHoras ? '<span style="color:#8C9196;font-size:11px;">leído el ' + fmtF(s.fechaHoras) + '</span>' : '') +
      '<input type="number" step="1" min="0" placeholder="horas hoy" id="' + id + 'Horas">' + btnH + 'Cargar lectura</button></div>';
    if (s.sinPlan) h += '<div class="mt-vacio">Este equipo no tiene plan de mantenimiento. Se carga una vez en <a href="mis-equipos.html" style="color:#B8731A;">Equipos y lotes → Mantenimiento</a> con el plan recomendado (Valley / Lindsay) y después el operador marca acá lo que va haciendo.</div>';
      ...
      s.tareas.slice().sort(function (a, b) { return orden[a.estado] - orden[b.estado] || (b.pct || 0) - (a.pct || 0); }).forEach(function (t) {
        ...
        h += '<div class="mt-tarea"><div class="n" title="' + esc((t.detalle ? t.detalle + ' ' : '') + (t.fuente ? 'Fuente: ' + t.fuente : '')) + '"><b>' + esc(t.tarea) + '</b><span>' + (t.componente ? esc(t.componente) + ' · ' : '') + esc(t.cadaTxt) + ' · ' +
          (t.ultFecha ? 'última ' + fmtF(t.ultFecha) + (t.ultHoras ? ' a las ' + fmt(t.ultHoras, 0) + ' h' : '') : (t.ultHoras > 0 ? 'sin registro, cuenta desde ' + fmt(t.ultHoras, 0) + ' h' : 'nunca registrada')) + (t.contador !== 'pivote' && t.intervalo && (cs[t.contador] || {}).propio ? ' · horas de ' + CONTADORES[t.contador].toLowerCase() : '') + (t.faltaTxt ? ' · ' + t.faltaTxt : '') + (t.sinVueltas ? ' · falta la hora por vuelta del pivot' : '') + '</span></div>' +
  ...
  function _horas(eqId, id) {
    var m = document.getElementById(id + 'Msg'), hechas = [];
    [['pivote', 'Horas'], ['bomba', 'HorasBomba'], ['canon', 'HorasCanon']].forEach(function (p) { var inp = document.getElementById(id + p[1]); if (inp && inp.value !== '') { var r = registrarHoras(eqId, inp.value, null, p[0]); if (r.ok) hechas.push(CONTADORES[p[0]].toLowerCase() + ' ' + fmt(r.ok.cantidad, 0) + ' h'); } });
    if (!hechas.length) { if (m) { m.style.color = '#B3261E'; m.textContent = 'Poné las horas que marca el horímetro.'; } return; }
    refrescar(id, eqId); var m2 = document.getElementById(id + 'Msg'); if (m2) { m2.style.color = '#178029'; m2.textContent = 'Lectura guardada: ' + hechas.join(' · ') + '.'; }
  }
  ...
  window.SafiaMant = { catalogoPara: catalogoPara, planDesdeCatalogo: planDesdeCatalogo, completarPlan: completarPlan, textoCada: textoCada, TEMPORADA: TEMPORADA, estado: estado, html: html, registrarHoras: registrarHoras, registrarHecha: registrarHecha, CONTADORES: CONTADORES, _horas: _horas, _hecha: _hecha };
})();
